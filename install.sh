...
#   curl -LsSf https://raw.githubusercontent.com/sync-dev-org/sync-sdd/main/install.sh | sh -s -- --version v1.14.1
...
if version_lt "$INSTALLED_VERSION" "0.10.0"; then
    ...
    # tasks.md files are left as-is; TaskGenerator will regenerate on next /sdd-roadmap impl run
    info "v0.10.0 migration complete."
    info "Note: Existing tasks.md files are preserved but will be regenerated by TaskGenerator on next /sdd-roadmap impl run."
fi
