...
5. `framework/claude/agents/` 配下の全ファイルを `.claude/agents/` にコピーする
...
2. skills, agents, rules, templates, profiles の全フレームワークファイルを上書きする
...
9. マーカー削除後の末尾空行を除去する
10. 空になったディレクトリ（`.claude/skills`, `.claude/commands`, `.claude/agents`, `.claude/sdd/settings`, `.claude/sdd`）を `rmdir` で削除する
...
3. `.claude/agents/` 配下の `sdd-*.md` ファイルについて、ソースに対応するファイルが存在しなければ削除する
...
5. **v0.10.0 マイグレーション（Format unification + Planner 廃止）:**
   ...
   - 既存の `tasks.md` は保持し、次回の `/sdd-impl` で TaskGenerator が再生成することを案内する
6. **v0.15.0 マイグレーション（commands → skills）:**
   - `.claude/commands/sdd-*.md` ファイルを全削除する
   - `.claude/commands/` ディレクトリが空になった場合、ディレクトリ自体を削除する
7. 各マイグレーションはバージョン順に連鎖実行する（v0.4.0 → v0.7.0 → v0.9.0 → v0.10.0 → v0.15.0）
8. 0.0.0（未インストール）からの新規インストール時は全マイグレーションが条件チェックで自動スキップされる
...
flowchart TD
    ...
    UN_LEGACY --> UN_AGENTS[agents/sdd-*.md 削除]
    ...
    READ_VER --> MIGRATION[マイグレーション実行<br/>v0.4.0 → v0.7.0 → v0.9.0<br/>→ v0.10.0 → v0.15.0]
...
|-----------|--------------|--------|---------------|------------------|
| Argument Parser | CLI | コマンドラインオプション解析 | 1, 2, 3, 4, 5, 10 | POSIX sh `case` |
...
| Framework | `.claude/agents/sdd-*.md` | 上書き | 削除 |
