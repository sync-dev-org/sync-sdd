...
**生成**: `/sdd-start` Step 5a で `date +%H%M%S` を実行し、出力をそのまま `$SID` とする (例: `104817`)。セッション（プロセス）ごとに一意な値となる。pane ID ベースだと同一 pane での再起動時に前セッションと衝突するため、時刻ベースを使用する。

**永続化**: SID と全 pane ID は `{{SDD_DIR}}/state.yaml` に記録される。pane タイトルは装飾用 (best-effort) であり、ロジックの依存先ではない。Claude Code は TUI/print 両モードで pane タイトルを上書きするため、タイトルに依存してはならない。
...
`/sdd-start` Step 5a で `$SID` 生成・Lead pane タイトル設定後に実行。
...
Lead はスロット状態を `{{SDD_DIR}}/state.yaml` の `grid` セクションで追跡。slot assign 時に status → busy + agent/engine/channel を記入、release 時に status → idle に戻す。
...
`/sdd-start` Step 5a で、SID 生成・Lead タイトル設定の**後**、Grid Creation の**前**に実行する。

**Primary (state.yaml ベース)**:
1. `{{SDD_DIR}}/state.yaml` を読む
2. 記載されている全 pane_id (lead + slots) について、`tmux list-panes -F '#{pane_id}'` で存在確認
3. 存在する pane を orphan 候補として収集（`$MY_PANE` = 自分の Lead pane は除外）
4. 存在しない pane はスキップ（既に消えている）
5. 孤児ペインが見つかった場合、**ユーザーに確認**: 「前セッション (SID: {old_sid}) の {N} 個の pane が残っています。kill しますか？」
6. ユーザーが承認 → Kill Pane。拒否 → skip

**Fallback (state.yaml なし)**: タイトルベースで検出。`tmux list-panes -F '#{pane_id} #{pane_title}'` (current window only) で `sdd-` prefix のタイトルを持つ pane を列挙し、SID が現在と異なるものを orphan 候補とする。
