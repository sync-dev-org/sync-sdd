...
**生成**: Session Resume Step 5a で `date +%H%M%S` を実行し、出力をそのまま `$SID` とする (例: `104817`)。セッション（プロセス）ごとに一意な値となる。pane ID ベースだと同一 pane での再起動時に前セッションと衝突するため、時刻ベースを使用する。

**Lead pane タイトル**: SID 生成直後に `tmux select-pane -T 'sdd-{SID}-lead'` を実行。他の Lead からの検出および Orphan Cleanup に使用。
...
Session Resume Step 5a で、SID 生成・Lead タイトル設定の**後**、Grid Creation の**前**に実行する。

1. List Panes, Grep for `sdd-` prefix のタイトルを持つペインを列挙（自分の Lead pane は除外）
2. 各タイトルから SID を抽出 (`sdd-{SID}-...` の `{SID}` 部分)
3. 抽出した SID ごとに `sdd-{SID}-lead` **タイトルを持つペイン**が現存するか確認（pane 存在ではなくタイトルで判定。プロセス再起動時に pane は残るがタイトルはリセットされるため）
4. `sdd-{SID}-lead` タイトルが見つかる → 別のアクティブセッション → skip。見つからない → 前セッションの孤児ペイン
