...
**永続化**: SID、window_id、全 pane ID は `{{SDD_DIR}}/state.yaml` に記録される。`window_id` は grid が存在するウィンドウを特定するために使用する（orphan 検出・grid 再利用のスコープ制御）。pane タイトルは装飾用 (best-effort) であり、ロジックの依存先ではない。Claude Code は TUI/print 両モードで pane タイトルを上書きするため、タイトルに依存してはならない。
...
Lead はスロット状態を `{{SDD_DIR}}/state.yaml` の `grid` セクションで追跡。`grid.window_id` は grid が存在するウィンドウを示す。slot assign 時に status → busy + 用途に応じた属性 (Pattern B: agent/engine/channel, Pattern A: agent/url) を記入、release 時に status → idle に戻し追加属性を除去する。
...
### Start
1. **Check for existing server**: `{{SDD_DIR}}/state.yaml` の `grid` セクションで `agent: {purpose}-*` の slot を検索。見つかった場合、そのサーバーを再利用 (retry 時にサーバーが前回から動いたまま)。`url` フィールドからサーバー URL を取得。
...
3. **Assign slot**: idle スロットを選択し、`send-keys` でサーバーコマンドを投入。`{{SDD_DIR}}/state.yaml` の該当 slot を更新:
   ```yaml
   status: busy
...
2. `{{SDD_DIR}}/state.yaml` の該当 slot を `status: idle` に更新し、`agent`/`url` を除去。
...
1. `{{SDD_DIR}}/state.yaml` を読む (`old_sid`, `grid.window_id`, 全 pane_id を取得)
