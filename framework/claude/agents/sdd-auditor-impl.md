...
Cross-check, verify, and integrate findings from up to 9 independent review agents into a unified, actionable implementation review report.
...
Read all `.cpf` files from the review directory. Each file contains one Inspector's findings in CPF format:
  ...
  7. `sdd-inspector-e2e.cpf` — E2E command execution: project-level E2E/integration test results (projects with E2E configured, may be absent)
  8. `sdd-inspector-web-e2e.cpf` — E2E functional testing: flow correctness, navigation, element visibility (web projects only, may be absent)
  9. `sdd-inspector-web-visual.cpf` — Visual design: design system compliance, aesthetics, accessibility (web projects only, may be absent)
