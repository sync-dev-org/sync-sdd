...
- **Avoid `$()` in Bash commands**: `$()` コマンド置換は Claude Code のセキュリティ検出で承認プロンプトが出る（allow リストで override 不可）。コミットメッセージは `git commit -m "メッセージ"` で直接渡す。HEREDOC+cat パターン (`git commit -m "$(cat <<'EOF'...)"`) は使わない。
