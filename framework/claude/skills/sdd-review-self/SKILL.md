...
**per-agent prompts**: テンプレートは `$TPL/` から直接読み込む。active/ へのコピーは不要。
動的プレースホルダー (`{{FOCUS_TARGETS}}`, `{{CACHED_OK}}`) はエンジン投入時にインライン置換する。

**エンジン起動時** (dispatch mode 別):
- **tmux/fallback**: Agent 1, 3 は `cat shared-prompt.txt $TPL/agent-{N}.md | $ENGINE_CMD` で直接投入。Agent 2, 4 は `sed 's|{{PLACEHOLDER}}|{value}|g' $TPL/agent-{N}.md | cat shared-prompt.txt - | $ENGINE_CMD` でワンライナー投入。
  **Note**: `{{FOCUS_TARGETS}}` と `{{CACHED_OK}}` の値は sed-safe であること（`&`、`|`、パイプ区切り文字を含まない）。改行を含む場合は Prep Agent が `$SCOPE_DIR/active/agent-{N}.md` に Write で書き出し、`cat shared agent-{N} | engine` で投入。
- **SubAgent**: Read `$TPL/agent-{N}.md` → 文字列内で `{{PLACEHOLDER}}` を置換 → shared-prompt.txt 内容と結合 → Agent prompt に渡す。Bash 不要。
...
3. 完了後の検証:
   ...
   - `$SCOPE_DIR/active/shared-prompt.txt` と `$SCOPE_DIR/active/focus-targets.txt` と `$SCOPE_DIR/active/cached-ok.txt` の存在を確認
...
エンジンバイナリ (`$ENGINE_CMD`) を組み立てる。Inspector dispatch では `$INSPECTOR_ENGINE_CMD` を使用（lead pipeline 時は base `$ENGINE_CMD`）。send-keys ではテンプレートを `$TPL/` から直接読み込み、`cat {shared} {$TPL/agent-N} | $INSPECTOR_ENGINE_CMD` の形でプロンプトを stdin に渡す。プレースホルダーがある場合は sed をパイプに挟む。
...
```
`{{FOCUS_TARGETS}}` / `{{CACHED_OK}}` が改行を含む場合は、Prep Agent (agent pipeline) または Lead (lead pipeline) が `$SCOPE_DIR/active/agent-{N}.md` に Write で書き出し、Agent 1,3 と同じ直接 cat パターンを使用する。
...
|-------|----------|-------------|----------|
| Prep | `$TPL/prep.md` | (none — all paths hardcoded) | agent only |
| 1 (Flow Integrity) | `$TPL/agent-1-flow.md` | (none) | both |
| 2 (Change-Focused) | `$TPL/agent-2-changes.md` | `{{FOCUS_TARGETS}}` | both |
| 3 (Consistency) | `$TPL/agent-3-consistency.md` | (none) | both |
| 4 (Compliance) | `$TPL/agent-4-compliance.md` | `{{CACHED_OK}}` | both |
| Auditor | `$TPL/auditor.md` | `{{ENGINE_INFO}}` | agent only |
...
2. 各失敗 Inspector について:
   - Read `$SCOPE_DIR/active/shared-prompt.txt` + `$TPL/agent-{N}-{name}.md` の内容を結合 (動的プレースホルダーは文字列置換)
