...
各ステージの engine に応じて `${STAGE}_ENGINE_CMD` を組み立てる (例: `$PREP_ENGINE_CMD`, `$INSPECTOR_ENGINE_CMD`, `$AUDITOR_ENGINE_CMD`)。send-keys ではテンプレートを `$TPL/` から直接読み込み、`cat {shared} {$TPL/agent-N} | ${STAGE}_ENGINE_CMD` の形でプロンプトを stdin に渡す。プレースホルダーがある場合は sed をパイプに挟む。
