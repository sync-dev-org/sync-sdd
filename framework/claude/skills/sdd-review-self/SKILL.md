...
外部エンジン (Codex CLI / Claude Code headless / Gemini CLI) を使った self-review スキル。4 Agent を外部エンジンで並行実行する。
...
6. 完了待ち: `tmux wait-for sdd-{SID}-review-self-prep-B{seq}` (background)
7. 完了後の検証:
...
3. Auditor を tmux slot (または background Bash) で dispatch:
   ...
   ```
4. 完了待ち: `tmux wait-for sdd-{SID}-review-self-auditor-B{seq}` (background)
5. 完了後の検証:
