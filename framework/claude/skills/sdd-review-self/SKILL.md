...
Bash: sleep 1.5; tmux send-keys -t {pane4} '...' Enter
```

4 Agent 分の send-keys 完了後:
1. send-keys ゾンビ確認: `pgrep -fl "tmux send-keys"` → 検出時はユーザーに報告し kill
2. 4 つの `tmux wait-for sdd-{SID}-review-self-{N}-B{seq}` も同様に staggered parallel dispatch (0.5 秒刻み) で並行発行し、全 Agent 完了を待つ。
