...
## Step 4: Parallel Dispatch (4 Inspectors)
...
- Channel = `sdd-{SID}-review-self-{N}-B{seq}` (`$SID` は Step 3 で取得したセッション固有 ID)。スロットの pane title (`sdd-{SID}-slot-{N}`) は変更しない。
...
MultiView スロットに `send-keys` で agent コマンドを投入する (Hold-and-Release パターン)。idle スロットから 4 つ選択し、`{{SDD_DIR}}/state.yaml` の該当 slot を `status: busy` + `agent`/`engine`/`channel` に更新。各 Agent の command chain を投入:
...
2. 4 つの `tmux wait-for sdd-{SID}-review-self-{N}-B{seq}` も同様に staggered parallel dispatch (0.5 秒刻み、`Bash(run_in_background=true)`) で並行発行:
```
...
Bash(run_in_background=true): tmux wait-for sdd-{SID}-review-self-4-B{seq}
```
...
**background mode** (上記以外):
`cat {shared} {active/agent-N-name.md} | {$INSPECTOR_ENGINE_CMD}` を 4 つの `Bash(run_in_background=true)` で並行実行。各 task-notification で個別に完了を検知。CPF はファイル書き出しで取得。
