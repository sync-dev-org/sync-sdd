...
各エンジンの stdin に `cat $SHARED_PROMPT $AGENT_PROMPT` で結合して渡す (`$SHARED_PROMPT` = `$SCOPE_DIR/active/shared-prompt.txt`, `$AGENT_PROMPT` = `$SCOPE_DIR/active/agent-{N}-prompt.txt`):

**codex**:
```
cat $SHARED_PROMPT $AGENT_PROMPT | npx -y @openai/codex exec --full-auto [--model $MODEL] -
```

**claude** (`env -u CLAUDECODE` で Lead セッションからのネスト検出を回避):
```
cat $SHARED_PROMPT $AGENT_PROMPT | env -u CLAUDECODE claude -p - --dangerously-skip-permissions [--model $MODEL]
```
ツール制限時: `--dangerously-skip-permissions` を `--allowedTools "$TOOLS"` に置換。

**gemini**:
```
cat $SHARED_PROMPT $AGENT_PROMPT | npx -y @google/gemini-cli -p "Review the project files per the instructions below." --yolo [--model $MODEL]
```
