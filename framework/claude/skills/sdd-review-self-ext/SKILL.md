...
argument-hint: "[--engine codex|claude|gemini] [--model <model-name>] [--timeout <seconds>] [--pipeline lead|agent]"
...
例: `/sdd-review-self-ext --pipeline agent`
...
| `$PIPELINE` | `--pipeline` arg → `roles.review-self.pipeline` → `lead` |
...
5. Report resolved config: `Engine: {$ENGINE_NAME} | Model: {$MODEL or "default"} | Timeout: {$TIMEOUT}s | Pipeline: {$PIPELINE}`

### Pipeline Mode

`$PIPELINE` determines work distribution:
- **lead** (default): Lead executes Steps 1-3 (prompt construction) and Step 7 (consolidation) inline
- **agent**: Prep Agent handles Steps 1-3, Auditor Agent handles Step 7. Lead only orchestrates dispatch and presents results

## Steps 1-3: Prompt Construction

```
$SCOPE_DIR = {{SDD_DIR}}/project/reviews/self-ext
$TPL = .claude/skills/sdd-review-self-ext/refs
```
...
3. プレースホルダを置換:
   `sed -i '' 's|__SCOPE_DIR__|{実際の $SCOPE_DIR}|g' $SCOPE_DIR/active/prep.md`
   `sed -i '' 's|__SDD_DIR__|{実際の {{SDD_DIR}}}|g' $SCOPE_DIR/active/prep.md`
   `sed -i '' 's|__TPL_DIR__|{実際の $TPL}|g' $SCOPE_DIR/active/prep.md`
4. Prep Agent を tmux slot (または background Bash) で dispatch:
   ```
   tmux send-keys -t {slot_pane_id} 'cat {$SCOPE_DIR}/active/prep.md | {$ENGINE_CMD}; tmux wait-for -S sdd-{SID}-ext-prep' Enter
   ```
5. 完了待ち: `tmux wait-for sdd-{SID}-ext-prep` (background)
6. 完了後の検証:
...
|-------|----------|-------------|----------|
| Prep | `$TPL/prep.md` | `__SCOPE_DIR__`, `__SDD_DIR__`, `__TPL_DIR__` | agent only |
...
| Auditor | `$TPL/auditor.md` | `__SCOPE_DIR__`, `__SDD_DIR__`, `__ENGINE_INFO__` | agent only |

Prep/Auditor テンプレートは `__PLACEHOLDER__` 形式を使用（agent テンプレートの `${PLACEHOLDER}` と区別するため、Lead の sed で Prep/Auditor のみ置換される）。
...
2. プレースホルダを置換:
   `sed -i '' 's|__SCOPE_DIR__|{実際の $SCOPE_DIR}|g' $SCOPE_DIR/active/auditor.md`
   `sed -i '' 's|__SDD_DIR__|{実際の {{SDD_DIR}}}|g' $SCOPE_DIR/active/auditor.md`
   `sed -i '' 's|__ENGINE_INFO__|{$ENGINE_NAME} [{$MODEL or "default"}]|g' $SCOPE_DIR/active/auditor.md`
3. Auditor を tmux slot (または background Bash) で dispatch:
   ```
   tmux send-keys -t {slot_pane_id} 'cat {$SCOPE_DIR}/active/auditor.md | {$ENGINE_CMD}; tmux wait-for -S sdd-{SID}-ext-auditor' Enter
   ```
4. 完了待ち: `tmux wait-for sdd-{SID}-ext-auditor` (background)
5. 完了後の検証:
