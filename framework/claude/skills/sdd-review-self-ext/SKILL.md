...
4. Prep Agent を tmux slot (または background Bash) で dispatch:
   ```
   tmux send-keys -t {slot_pane_id} 'cat {$SCOPE_DIR}/active/prep.md | {$PREP_ENGINE_CMD}; tmux wait-for -S sdd-{SID}-ext-prep-B{seq}' Enter
   ```
5. send-keys ゾンビ確認 (tmux mode): `pgrep -fl "tmux send-keys"` → 検出時はユーザーに報告し kill
6. 完了待ち: `tmux wait-for sdd-{SID}-ext-prep-B{seq}` (background)
7. 完了後の検証:
...
- Channel = `sdd-{SID}-ext-{N}-B{seq}` (`$SID` は Step 4 で生成したセッション固有 ID)。スロットの pane title (`sdd-{SID}-slot-{N}`) は変更しない。
...
3. Auditor を tmux slot (または background Bash) で dispatch:
   ```
   tmux send-keys -t {slot_pane_id} 'cat {$SCOPE_DIR}/active/auditor.md | {$AUDITOR_ENGINE_CMD}; tmux wait-for -S sdd-{SID}-ext-auditor-B{seq}' Enter
   ```
