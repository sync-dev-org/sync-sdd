...
各 Inspector について `{{OUTPUT_PATH}}` を展開した preamble を `{scope-dir}/active/preamble-{inspector-name}.md` に生成:

```
sed "s|{{OUTPUT_PATH}}|{scope-dir}/active/{inspector-name}.cpf|g" {scope-dir}/active/shared-preamble.md > {scope-dir}/active/preamble-{inspector-name}.md
```
...
全 Inspector を並行 dispatch (単一メッセージで parallel tool call):
```
...
send-keys 完了後:
1. ゾンビ確認: `pgrep -fl "tmux send-keys"` → 検出時はユーザーに報告し kill
...
```
全 Inspector を並列 `Bash(run_in_background=true)` で実行。
...
1. 失敗 Inspector を `Agent(subagent_type="sdd-inspector-{name}", model="sonnet", run_in_background=true, prompt="{context preamble}")` で再 dispatch
...
1. `Agent(subagent_type="sdd-auditor-{type}", model="sonnet", run_in_background=true, prompt="{auditor context}")` で再 dispatch
