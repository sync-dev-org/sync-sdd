...
idle slot を選択し、`{{SDD_DIR}}/state.yaml` の該当 slot を `status: busy` + `agent: prep`/`engine`/`channel` に更新。
```
tmux send-keys -t {slot_pane_id} 'cat {$TPL}/prep.md | {$PREP_ENGINE_CMD}; tmux wait-for -S sdd-{SID}-review-prep-B{seq}' Enter
```
send-keys ゾンビ確認後、`Bash(run_in_background=true)` で `tmux wait-for sdd-{SID}-review-prep-B{seq}` を実行。task-notification で完了を検知。完了後、state.yaml の該当 slot を `status: idle` に戻す。
...
MultiView スロットに `send-keys` で agent コマンドを投入 (Hold-and-Release パターン)。idle スロットを選択し、`{{SDD_DIR}}/state.yaml` の該当 slot を `status: busy` + `agent`/`engine`/`channel` に更新:
```
...
2. `{{SDD_DIR}}/state.yaml` の該当 slot を `status: idle` に更新し、`agent`/`engine`/`channel` を除去。
...
idle スロットを選択し、`{{SDD_DIR}}/state.yaml` の該当 slot を `status: busy` + `agent: auditor`/`engine`/`channel` に更新。
```
...
```
`Bash(run_in_background=true)` で `tmux wait-for sdd-{SID}-review-auditor-B{seq}` を実行。task-notification で完了を検知。完了後、state.yaml の該当 slot を `status: idle` に更新。
...
2. Verdict を `{scope-dir}/verdicts.md` に永続化:
   ...
   b. Batch entry header 追加:
      ...
      - Cross-cutting: `## [CC-B{seq}] {review-type} | {ISO-8601} | v{version}`
