...
idle slot を選択し、`{{SDD_DIR}}/state.yaml` の該当 slot を `status: busy` + `agent: briefer`/`engine`/`channel` に更新。pane タイトルを更新: `tmux select-pane -t {pane_id} -T "review/briefer | {$BRIEFER_MODEL}"`
```
...
MultiView スロットに `send-keys` で agent コマンドを投入 (Hold-and-Release パターン)。idle スロットを選択し、`{{SDD_DIR}}/state.yaml` の該当 slot を `status: busy` + `agent`/`engine`/`channel` に更新。pane タイトルを更新: `tmux select-pane -t {pane_id} -T "review/{inspector-name} | {$INSPECTOR_MODEL}"`:
```
