...
オプション:
- `--prep-engine <name>` / `--prep-model <name>`: Prep エンジン/モデル override
...
### Design Review
Fixed (5): `design-rulebase`, `design-testability`, `design-architecture`, `design-consistency`, `design-best-practices`
Dynamic (1-4): Prep が生成 (Step 3b)

Auditor: `auditor`

### Implementation Review
Fixed (5): `impl-rulebase`, `impl-interface`, `impl-test`, `impl-quality`, `impl-consistency`

Conditional:
- `impl-e2e`: `steering/tech.md` Common Commands に `# E2E` + 非空・非 placeholder コマンドが存在する場合
- `impl-web-e2e` + `impl-web-visual`: `steering/tech.md` に web stack indicators (React, Next.js, Vue, Angular, Svelte, Express, Django+templates, Rails, FastAPI+frontend 等) が存在する場合
...
### Dead-Code Review
Fixed (4): `dead-settings`, `dead-code`, `dead-specs`, `dead-tests`
Dynamic: なし

Auditor: `auditor`

### 3b: Prep Dispatch (design / impl のみ)

Dead-code review ではスキップ。

Prep Agent を dispatch して Dynamic Inspector プロンプトを生成。分岐順序: SubAgent → tmux → background (Inspector/Auditor と統一)。

**SubAgent mode** (`$PREP_ENGINE == "subagents"`):
```
Agent(subagent_type="general-purpose", description="Prep for {REVIEW_TYPE} review", run_in_background=true, prompt="
Read {$TPL}/prep.md and execute the instructions.
...
**tmux mode** (`$PREP_ENGINE != "subagents"` かつ `$TMUX` 設定あり):
idle slot を選択し、`{{SDD_DIR}}/state.yaml` の該当 slot を `status: busy` + `agent: prep`/`engine`/`channel` に更新。pane タイトルを更新: `tmux select-pane -t {pane_id} -T "prep"`
```
tmux send-keys -t {slot_pane_id} 'cat {$TPL}/prep.md | {$PREP_ENGINE_CMD}; tmux wait-for -S sdd-{SID}-review-prep-B{seq}' Enter
```
ユーザーに報告: `Prep dispatched to slot-{N} ({pane_id})`
send-keys ゾンビ確認後、`Bash(run_in_background=true)` で `tmux wait-for sdd-{SID}-review-prep-B{seq}` を実行。task-notification で完了を検知。完了後、state.yaml の該当 slot を `status: idle` に戻し、`agent`/`engine`/`channel` を除去。pane タイトルをリセット: `tmux select-pane -t {pane_id} -T "slot-{N} idle"`

**background mode** (上記以外):
`Bash(run_in_background=true)` で `cat {$TPL}/prep.md | {$PREP_ENGINE_CMD}` を実行。task-notification で完了を検知。

完了後:
1. `{scope-dir}/active/dynamic-manifest.md` を Read
...
3. Step 5 以降で fixed + dynamic を統一的に dispatch
...
## Step 5: Context Preamble Generation
...
`{{DENY_PATTERNS}}` 展開: `$DENY_PATTERNS` (Step 1b でロード済み) を改行区切りでフォーマットし、shared-preamble.md 内の `{{DENY_PATTERNS}}` を置換。SubAgent mode でも展開する (preamble は外部エンジンのみで使用するが、テンプレートの一貫性のため)。
...
### 5c Auditor Preamble Generation
...
Web inspectors (`impl-web-e2e`, `impl-web-visual`) を含む場合、Inspector dispatch **前に** dev server を起動。
...
6. Per-inspector preamble を再生成 (shared-preamble が更新されたため、Step 5b を再実行)
...
MultiView スロットに `send-keys` で agent コマンドを投入 (Hold-and-Release パターン)。idle スロットを選択し、`{{SDD_DIR}}/state.yaml` の該当 slot を `status: busy` + `agent`/`engine`/`channel` に更新。pane タイトルを更新: `tmux select-pane -t {pane_id} -T "{inspector-name}"`:
```
tmux send-keys -t {slot_pane_id} 'cat {scope-dir}/active/preamble-{name}.md .sdd/settings/templates/review/{name}.md | {$INSPECTOR_ENGINE_CMD}; tmux wait-for -S sdd-{SID}-review-{name}-B{seq}; tmux wait-for sdd-{SID}-close-B{seq}' Enter
```
...
**background mode** (上記以外):
```
Bash(run_in_background=true): cat {scope-dir}/active/preamble-{name}.md .sdd/settings/templates/review/{name}.md | {$INSPECTOR_ENGINE_CMD}
```
...
Auditor dispatch 前に、Step 5c で生成済みの Auditor preamble 内 `{{UNAVAILABLE_INSPECTORS}}` を `$UNAVAILABLE_INSPECTORS` (Step 6d で記録) で展開:
```
sed -i '' "s|{{UNAVAILABLE_INSPECTORS}}|{$UNAVAILABLE_INSPECTORS}|g" {scope-dir}/active/preamble-{auditor-name}.md
```
...
idle スロットを選択し、`{{SDD_DIR}}/state.yaml` の該当 slot を `status: busy` + `agent: auditor`/`engine`/`channel` に更新。pane タイトルを更新: `tmux select-pane -t {pane_id} -T "auditor"`
```
tmux send-keys -t {slot_pane_id} 'cat {scope-dir}/active/preamble-{auditor-name}.md .sdd/settings/templates/review/{auditor-name}.md | {$AUDITOR_ENGINE_CMD}; tmux wait-for -S sdd-{SID}-review-auditor-B{seq}' Enter
```
...
**background mode** (上記以外):
`Bash(run_in_background=true)` で `cat {scope-dir}/active/preamble-{auditor-name}.md .sdd/settings/templates/review/{auditor-name}.md | {$AUDITOR_ENGINE_CMD}` を実行。task-notification で完了を検知。
...
1. 失敗 Inspector を `Agent(subagent_type="general-purpose", description="{inspector-name} fallback", run_in_background=true, prompt="Read .sdd/settings/templates/review/{inspector-name}.md and execute. {context preamble}")` で再 dispatch。task-notification で完了を検知
...
2. Verdict を `{scope-dir}/verdicts.md` に永続化:
   ...
   b. Batch entry header 追加 (`date +%Y-%m-%dT%H:%M:%S%z` で ISO-8601 timestamp 取得、`v{version}` は VERSION ファイルの値、engine 情報は engines.yaml の model 値を使用):
      ...
      - Cross-cutting: `## [CC-B{seq}] {review-type} | {ISO-8601} | v{version} | prep:{model} insp:{model} aud:{model} | fixed:{N} conditional:{N} dynamic:{N}`
