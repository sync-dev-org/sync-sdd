...
- **5 SubAgents + 21 review templates, 3 tiers** — Analyst redesigns from zero, Architect designs features, TaskGenerator decomposes into parallel tasks, Builders implement with TDD, ConventionsScanner ensures codebase consistency. 5 fixed Inspectors + 1-4 dynamic Inspectors review from different angles, Auditor synthesizes the verdict. Dynamic Inspectors are generated by a Briefer that analyzes the review target to identify change-specific risks. Review agents use engine-agnostic templates dispatched via SubAgent or external CLI (Codex, Claude, Gemini).
...
curl -LsSf <url>/install.sh | sh -s -- --version v2.7.0
